...
// writeTempScenario marshals v to a temp JSON file and registers cleanup.
...
func TestLoadScenarioMissingFileReturnsError(t *testing.T) {
	_, err := LoadScenario("/no/such/file.json")
	if err == nil {
...
func TestGearDefaultsToForwardWhenOmitted(t *testing.T) {
	// Old JSON with no gear_position in defaults
	path := writeTempScenario(t, minimalValidScenario(
...
func TestGearZeroInDefaultsBecomesForward(t *testing.T) {
	// Explicitly setting gear_position=0 in defaults is treated as "not set" → 1
	path := writeTempScenario(t, minimalValidScenario(
		map[string]interface{}{"gear_position": 0},
		nil,
	))
	scen, _ := LoadScenario(path)
	if scen.Defaults.GearPosition != 1 {
...
func TestEvalSegmentBeyondAllSegmentsUsesDefaults(t *testing.T) {
	...
	eval := EvalSegment(&scen, 9.0) // after all segments
...
func TestLoadRealFullCruiseScenario(t *testing.T) {
	scen, err := LoadScenario("scenarios/full_cruise_maneuver_pid.json")
	if err != nil {
...
func TestLoadRealCuspingScenario(t *testing.T) {
	...
	scen, err := LoadScenario("scenarios/cusping_maneuver_pid.json")
	if err != nil {
...
func TestLoadRealGentleSlalomScenario(t *testing.T) {
	scen, err := LoadScenario("scenarios/gentle_slalom_pid.json")
	if err != nil {
