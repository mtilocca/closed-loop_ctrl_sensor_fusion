...
clothoid arc smoothing, and produces a Go runner–compatible JSON scenario.
Optionally refines the trajectory with CasADiOptimizer.

_PoseParser is a thin argparse.ArgumentParser subclass that handles negative
coordinate values such as '--goal -20,0,0' without misidentifying them as
unknown flags.
...
class PathPlanner:
    ...
    applies clothoid arc smoothing, and produces a Go runner JSON scenario.
        ...
        for leg_idx in range(len(chain) - 1):
            ...
            if obstacle_map is None:
                ...
                leg_path = self.astar.plan(
                    ...
                    grid_res=grid_res, obstacle_map=obstacle_map,
                )
                if leg_path is not None:
